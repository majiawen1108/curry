...
/**
 * @author jw.ma
 * @title: MybatisSqlLoggerInterceptor
 * @description: 拦截StatementHandler类中参数类型为Statement的 prepare 方法,
...
//        log.info("类型 ==> " + sqlCommandType);
        ...
        long time = (end - start);
        // 获取到最终的sql语句
        String newsql = getSql(configuration, boundSql, id,time);
        log.info(newsql);
        return returnValue;
    }
    ...
    private String getSql(Configuration configuration, BoundSql boundSql, String sqlId,long time) {
        ...
        str.append(sqlId).append(":").append(sql).append(":").append("耗时").append(" ").append(time).append("ms");
        return str.toString();
    }
